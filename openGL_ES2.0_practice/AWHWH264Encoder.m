...
@property (nonatomic,assign) VTCompressionSessionRef vEnSession;
...
-(aw_flv_video_tag *)encodeYUVDataToFlvTag:(NSData *)yuvData
{
    ...
    uint32_t ptsMs = self.manager.timestamp + 1;
    CMTime pts = CMTimeMake(ptsMs, 1000);
    
    NSLog(@"start encode video = %p,ptsMs = %@",pixelBuf,@(ptsMs));
                                         ...
                                         CM_NULLABLE CMSampleBufferRef sampleBuffer)
{
    ...
    NSLog(@"end encode video = %p, isKeyFrame = %d",sourceFrameRefCon,isKeyFrame);
    
    //首先获取sps 和pps
    //sps pps 也是h264的一部分，可以认为它们是特别的h264视频帧，保存了h264视频的一些必要信息。
    //没有这部分数据h264视频很难解析出来。
