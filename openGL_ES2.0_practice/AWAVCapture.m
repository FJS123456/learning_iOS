...
- (void)updateFps:(NSInteger)fps
{
    ...
    for (AVCaptureDevice *vDevice in videoDevices) {
        float maxRate = [[vDevice.activeFormat.videoSupportedFrameRateRanges objectAtIndex:0] maxFrameRate];
        if (maxRate >= fps) {
...
{
    [self sendVideoYuvData:videoData toEncodeQueue:self.encodeSampleOpQueue toSendQueue:self.sendSampleOpQueue];
}
...
-(void) sendSpsPpsAndAudioSpecificConfigTagToSendQueue:(NSOperationQueue *) sendQueue
{
    ...
    [sendQueue addOperationWithBlock:^{
        ...
        if (spsPpsTag) {
//            aw_streamer_send_video_data(spsPpsTag);
        }
